...
- 해당 프로젝트에서는 식단 추천에서 중요한 기준중 하나인 '고기' 카테고리에 대해 분류 진행 (식단 메뉴에 어떤 고기가 들어가는지 분류).   
...
- #### 2nd Case 
  ...
  - **`Predict()`** 함수 선언 
    - 모델에 메뉴 입력 -> 라벨링 결과 출력 
  - **`Predict()`** 함수를 통해 'Nuvi_Foods' 데이터와 결과 비교  (595/1855 고기에 대해 라벨링 완료)
    ...
    ![스크린샷 2022-08-03 오후 1 16 19](https://user-images.githubusercontent.com/90128775/182523089-07a1f4d2-3994-4a4a-8856-195d4f69ea47.png)
...
### 2.4. 고려했던 모델들 
- 시도도 해보고 디버깅도 해보았으나 프로젝트의 시간 관계상 적용하지 못했던 모델들
- #### Autokeras 
...
### 보완점 
- 단어 리스트 업데이트를 통한 성능 개선 가능
- KoBERT는 이런 분류 작업에 사용하기에는 너무 복잡한 모델; KoBERT와는 다르게 ‘음식' 단어 자체에 대해 학습이 더 잘 되는 모델이 있나 확인 필요 
- KoBERT 가 아닌 다른 방식으로 필터링과 라벨링을 (직접적인 라벨링) 진행하여  그 결과를 기반으로 식단을 추천하는 모델을 만든다 

### 개인적인 회고 
- 모델링과 디버깅에 시간을 뺏겨서 원하는 결과물을 도출하는데 어려움이 있었음 
- 접근법: 이 모델, 저 모델을 시도해보기 전에 사전에 사용 가능한 모델들을 검토했으면 좋았을 것 같다
  - 처음 접하는 모델들이라서 일단은 한번 모델을 적용시켜보자는 생각이 강했음   

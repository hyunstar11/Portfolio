...
- **`목적`**: 암호화폐 시장에서 인덱스 지표와도 같은 같은 비트코인의 가격 방향성을 예측하는 딥러닝 모델을 만드는 것 (가격의 상방 혹은 하방) 
  ...
  - **`Q. 왜 트위터 데이터?`** : 흔히 자산시장에서 '인간지표' 라는 말을 접할 수 있; 어떤 사람이 주식이나 암호화폐를 매수하면 그 주식이나 암호화폐의 시세가 빠지는 경우가 있는데, 이런 사람을 두고 인간지표라고 지칭한다 (일종의 밈이라고 이해를 하면 되겠다. 유투버 박00 나 사0 처럼. 누구의 포지션 반대로 가면 꿀이다라고 해서 0반꿀 이라는 신조어까지 생겼다). 그렇다면 이 '인간지표'를 수치화 할 수 있는 방법은 없을까? 인간지표를 통해 투자에서 인사이트를 제공할 수 있을까? 하는 생각에서 출발
- 궁극적으로는 트위터 감성 분석을 통해 일종의 트레이딩뷰 인디케이터를 만드는 걸 목표로 프로젝트를 시작
  - 예) 트레이딩 뷰에서 접할 수 있는 Wave Trend, Super Trend 등과 같이 알고리즘을 기반으로 롱/숏 진입 타점을 알려주는 방식  
...
- 시장의 Sentiment를 통해 비트코인 가격의 방향성을 예측했을 때 0.5 accuracy를 보임  
  - 상하방을 예측하는 데 있어서 반반의 확률이기에 유의미한 결과는 아님  
...
![스크린샷 2022-08-25 오후 9 37 25](https://user-images.githubusercontent.com/90128775/186669746-5486a81b-00ff-419b-8012-b93da63f81c5.png)
  ...
  - 즉 Sentiment가 폭발하는 시기에는 변동성에 익숙하지 않은 사람 / 특정 방향에 대해 과한 레버리지를 짊어지고 있는 투자자는 레버리지를 줄이는 게 좋은 판단일 것으로 보임 
...
- 향후 발전방향 
  ...
  - LSTM 말고 더 최신의 모델을 사용? 
    - ARIMA (타임시리즈 예측에 좋은 성능) 
  - Sentiment ~ 가격/가격 방향 관계에 대한 수치화
    - DL을 잘 모르는 사람도 결과를 쉽게 이해할 수 있도록 변경  
  - DL 모델 성능 개선 
    - 하이퍼파라미터 튜닝 필요 
- Real Time Application에 대한 고민 필요 
  - 트위터 데이터를 실시간으로 프로세스하여 트레이딩뷰 차트를 통해 High Volatility를 암시할 수 있는 인디 발전시키는 방향 
    - 가격 방향성 예측에 대한 정확도가 높아진다면 Long / Short 옵션도 추가    

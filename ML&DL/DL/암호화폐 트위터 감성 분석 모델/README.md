...
- **`목적`**: 암호화폐 시장에서 인덱스 지표와도 같은 같은 비트코인의 가격 방향성을 예측하는 딥러닝 모델을 만드는 것 (가격의 상방 혹은 하방) 
  ...
  - **`Q. 왜 트위터 데이터?`** : 흔히 자산시장에서'인간지표' 라는 말을 접하게 된다; 어떤 사람이 주식이나 암호화폐를 매수하면 그 주식이나 암호화폐의 시세가 빠지는 경우가 있는데, 이런 사람을 두고 인간지표라고 지칭한다 (일종의 밈이라고 이해를 하면 되겠다. 유투버 박00 나 사0 처럼. 누구의 포지션 반대로 가면 꿀이다라고 해서 0반꿀 이라는 신조어까지 생겼다). 그렇다면 이 '인간지표'를 수치화 할 수 있는 방법은 없을까? 인간지표를 통해 투자에서 인사이트를 제공할 수 있을까? 하는 생각에서 출발
- 궁극적으로는 트위터 감성 분석을 통해 일종의 트레이딩 알고리즘에 적용을 하는 걸 목표로 프로젝트를 시작하였음 
...
![스크린샷 2022-08-25 오후 9 37 25](https://user-images.githubusercontent.com/90128775/186669746-5486a81b-00ff-419b-8012-b93da63f81c5.png)
  ...
  - 즉 Sentiment가 폭발하는 시기에는 변동성에 익숙하지 않은 사람들은 레버리지를 줄이면서 리스크 관리가 필요할 것으로 보임 
...
- 향후 발전방향 
  ...
  - DL 모델 성능 개선 
    - 하이퍼파라미터 튜닝 필요 
- Real Time Application에 대한 고민도 필요 
  - 트위터 데이터를 실시간으로 프로세스하여 트레이딩뷰 차트를 통해 High Volatility를 암시할 수 있는 Indicator로 발전시키는 방향 

...
### 성능 
- 누비푸드 데이터의 라벨링에 비해서 다소 아쉬운 퍼포먼스의 원인은 무엇인가?
- 데이터 누수의 문제가 있었나?
  1. Target: 예측 시점에서 사용할 수 없는 데이터가 있나? (없다고 판단)
  2. Train: Test Contamination - 데이터가 제대로 구별되지 않았나? (없다고 판단)
- 음식 단어들에 대한 특징 학습이 잘 이루어지지 않음; 새로운 메뉴에 대해서 분류가 안 됨 
- 즉, 단어 리스트 외의 단어들에 대한 분류가 어려움 
